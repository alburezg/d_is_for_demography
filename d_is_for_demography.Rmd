...
subtitle: "Christmas trivia edition"
...
pop %>% 
  ...
  mutate(location = ifelse(location == "Nigeria", "Nigeria", "Japan")) %>% 
...
ppr <- ppr %>% 
  ...
  select(-measure)
ppr %>% 
  mutate(name = factor(parity)) %>% 
  ggplot(aes(cohort, ratio, color = name)) + 
  geom_line(lwd = 1.1) + 
  scale_color_viridis_d() + 
  ylab("PPR") + 
  theme_gray(base_size = 14) + 
  labs(caption = "Data from HFD") +
  theme_bw(base_size = 14)
```
...
```{r}
lt <- read_table("data/lt.txt", skip = 2)
lt <- lt %>% 
...
lt %>% 
  ...
  xlab("age") + 
  theme_bw(base_size = 14) + 
  labs(title = "Ontario, 2015", caption = "data from the Canadian Human Mortality Database")
```
...
df %>% 
  ...
  theme_gray(base_size = 14) + 
  labs(title = "Selected countries, 1950-2015", caption = "Data from WPP 2019") + 
  theme_bw(base_size = 14)
```
...
lt %>%
  filter(x<20) %>% 
  select(x,n, lx, dx, ax, Lx, Tx, ex) %>% kable(booktabs = T,caption = 'Ontario life table, 2015 (Source: CHMD)') %>% 
